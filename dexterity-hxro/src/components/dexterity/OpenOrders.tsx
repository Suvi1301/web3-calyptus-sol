...
export const OpenOrders: FC = () => {
  ...
  const cancelOrders = useCallback(async () => {
    ...
    try {
      ...
      notify({
        type: "error",
        message: `Error canceling all orders, ${error}`,
      });
    } finally {
  ...
  return (
    <>
      <div className="rounded-lg border border-white p-4">
        <h1 className="mb-4 text-2xl">Orders Info</h1>
        <Button
          text={!requested ? `Cancel All Orders` : `Cancelling all orders...`}
                ...
                {orderData &&
                  ...
                  orderData.map((order, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        {parseFloat(order.price.m) *
                          Math.pow(10, parseInt(order.price.exp))}
                      </td>
                      <td>
                        {parseFloat(order.qty.m) /
                          Math.pow(10, parseInt(order.qty.exp))}
                      </td>
