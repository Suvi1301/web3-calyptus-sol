...
import {
  dexterity,
  TraderAccount,
  TraderAccountDropdownProps,
} from "../../utils/dexterityTypes";
...
export const SelectTraderAccounts: FC = () => {
  ...
  const fetchTraderAccounts = useCallback(async () => {
    ...
    if (!manifest.fields) {
      console.log("manifest.fields error");
      return;
    }
    if (!manifest.fields.wallet.publicKey) {
      console.log("manifest.fields.wallet.publicKey error");
      return;
    }
    ...
    try {
      ...
      notify({
        type: "error",
        message: `Selecting Trader Account failed!`,
  ...
  const handleCreateTRG = useCallback(async () => {
    try {
      ...
      notify({
        type: "error",
        message: `Creating Trader Account failed!`,
        description: error?.message,
      });
    }
  ...
  return (
    <div className="mt-4 flex flex-col items-center justify-center rounded-lg border border-white p-4">
      <h1 className="mb-4 text-2xl">Select or Create a Trader Account</h1>
      ...
      {trgsArr.length > 0 ? (
        <div className="flex w-full flex-col items-center space-y-4">
          ...
          <Button
            ...
            disabled={!publicKey}
            className={`text-md w-full rounded-md ${publicKey ? "bg-gradient-to-br from-[#80ff7d] to-[#80ff7d] text-black hover:from-white hover:to-purple-300" : "cursor-not-allowed bg-gray-300"}`}
          />
        </div>
      ) : (
        <div className="flex w-full flex-col items-center space-y-4">
  ...
  return (
    ...
    >
      ...
      {accounts.map((trg, index) => (
        <option key={index} value={trg.pubkey.toBase58()}>
